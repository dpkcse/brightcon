...
<section class="container section-spacing">
    ...
    @if($images->count())<div class="gallery-grid">@foreach($images as $image)@php($imageUrl = FrontendImage::url($image->image))<figure class="gallery-card">@if($imageUrl)<img src="{{ $imageUrl }}" alt="{{ $image->title ?: $image->category ?: 'Gallery image' }}" loading="lazy" decoding="async">@else<div class="image-placeholder">Gallery</div>@endif<figcaption><strong>{{ $image->title ?: 'Project Image' }}</strong><span>{{ $image->category ?: 'Gallery' }}</span></figcaption></figure>@endforeach</div><div class="mt-5">{{ $images->links() }}</div>@else @include('frontend.partials.content.empty-state', ['title' => 'No gallery images found', 'message' => 'Active gallery images added from the CMS will appear here.']) @endif
</section>
@endsection
