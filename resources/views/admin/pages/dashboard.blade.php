...
@section('content')
<div class="row g-4">
 @foreach([['Total Projects',$totalProjects],['Active Projects',$activeProjects],['Featured Projects',$featuredProjects],['Total Services',$totalServices],['Active Services',$activeServices],['Gallery Images',$totalGalleryImages],['Active Gallery Images',$activeGalleryImages],['Contact Messages',$totalContactMessages],['Unread Messages',$unreadContactMessages]] as [$label,$value])
 ...
 <div class="col-lg-5"><div class="card border-0 shadow-sm"><div class="card-header bg-white fw-semibold">Quick Links</div><div class="card-body d-grid gap-2">@foreach([['General Settings',route('admin.settings.general.edit')],['Theme Settings',route('admin.settings.theme.edit')],['Menu Items',route('admin.menu-items.index')],['Social Links',route('admin.social-links.index')],['Footer Links',route('admin.footer-links.index')],['Projects',route('admin.projects.index')],['Services',route('admin.services.index')],['Gallery Images',route('admin.gallery-images.index')]] as [$label,$url])<a class="btn btn-outline-primary-brand text-start" href="{{ $url }}">{{ $label }}</a>@endforeach</div></div></div>
</div>
@endsection
